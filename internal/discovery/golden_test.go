...
func TestGoldenNutrition(t *testing.T) {
	...
	if sp.Host != "nutrition.example.org" {
...
func TestGoldenTravels(t *testing.T) {
	...
	if sp.Host != "travels.example.org" {
		t.Errorf("host = %q, want travels.example.org", sp.Host)
	}
...
func TestGoldenGitea(t *testing.T) {
	...
	want := []string{"https://git.example.org/user/oauth2/authentik/callback"}
