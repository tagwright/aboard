...
func testCfg() *config.Config {
	...
	c.Authentik.PublicURL = "https://auth.example.org"
...
func travelsFull() map[string]string {
	m := travelsMixed()
	m["traefik.http.routers.travels-outpost.rule"] = "Host(`travels.example.org`) && PathPrefix(`/outpost.goauthentik.io/`)"
...
func TestVerify(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			sp:              faSpec("travels", "travels.example.org", "does-not-exist"),
			labels:          travelsMixed(),
...
func TestVerifyOIDCSkips(t *testing.T) {
	cfg := testCfg()
	sp := faSpec("gitea", "git.example.org")
...
func TestVerifyDefaultProtectsEveryHostRouter(t *testing.T) {
	...
	cfg := testCfg()
	sp := faSpec("travels", "travels.example.org") // no named routers
	res := Verify(cfg, sp, travelsMixed(), false)
