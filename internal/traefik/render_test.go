...
func TestRenderServiceMixedHost(t *testing.T) {
	...
	cfg := testCfg()
	sp := faSpec("travels", "travels.example.org", "travels-admin")
	...
	wantContains := []string{
		...
		"traefik.http.routers.travels-outpost.rule=Host(`travels.example.org`) && PathPrefix(`/outpost.goauthentik.io/`)",
...
func TestRenderSetupV3(t *testing.T) {
	...
	if !strings.Contains(out, "HostRegexp(`^[a-z0-9-]+\\.example\\.org$`)") {
...
func TestRenderSetupV2(t *testing.T) {
	...
	if !strings.Contains(out, "HostRegexp(`{subdomain:[a-z0-9-]+}.example.org`)") {
...
func TestFleetDomainDerivation(t *testing.T) {
	cfg := testCfg()
	cfg.Authentik.PublicURL = "https://auth.example.org"
	d, guessed := fleetDomain(cfg)
	if d != "example.org" {
		t.Errorf("fleetDomain = %q, want example.org", d)
	}
	if !guessed {
